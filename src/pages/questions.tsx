    ...
    >
      <main>
        <Hero
          ...
          tagline="Answers to Technical GeoDa Questions"
        ...
        <section className="main-content">
          <div style={{ maxWidth: '64rem', padding: '2rem 6rem', margin: '0 auto', fontSize: '1.1rem', lineHeight: '1.6' }}>
            <ul>
              ...
              <li><a href="#smooth">How to assess the sensitivity of different smoothers?</a></li>
            ...
            <h2 id="data">What characteristics should my data have?</h2>
            ...
            <h2 id="datasize">What are data size limits in GeoDa?</h2>
            ...
            <h2 id="specs">What platforms does GeoDa run on?</h2>
            <p>GeoDa runs on Windows, Mac OSX and Linux. It does not access restricted system folders in Windows.</p>

            <h2 id="layer">Can I load more than one layer in GeoDa?</h2>
            ...
            <h2 id="dist">What happens when I set the threshold distance lower than the default?</h2>
            <p>The default distance threshold ensures that every observation has at least one neighbor. If you set the threshold distance as smaller than the default minimum, GeoDa will create a <a href="./glossary#wghts">weights matrix</a> based on your threshold, not the default, i.e., it will contain "island" observations without neighbors. You can easily check this by looking at the weights characteristics in GeoDa and by opening the weights matrix in a text editor.</p>
            <p>If you are curious about the difference between the matrix with the default minimum threshold and yours, create both matrices and compare them by looking at their <a href="./glossary#hist">weights characteristics</a> (which is a histogram of the number of neighbors for all observations). You will see that the minimum threshold matrix has no observations with zero neighbors while yours will have observations with zero neighbors.</p>

            <h2 id="invdist">Does GeoDa include inverse distance weighting?</h2>
            <p>Yes. You can create different types of distance-based weights (with .gwt and .kwt file extensions) in GeoDa:</p>

            <p>GeoDa implements weights based on distance bands, inverse distance weighting and kernel weighting. For distance bands, the third column with distances in the .gwt file is only used to determine whether a point is within a distance band or not. The extent of the distance band is specified by your threshold distance (the default value ensures that you have no islands). GeoDa row-standardizes weights, i.e., the rows sum to 1. You can find more information about distance-based spatial weights <a href="https://geodacenter.github.io/workbook/4b_dist_weights/lab4b.html" target="_blank" rel="noopener noreferrer">here</a> and about inverse distance weights and kernel weights <a href="https://geodacenter.github.io/workbook/4c_distance_functions/lab4c.html" target="_blank" rel="noopener noreferrer">here</a>.</p>
            ...
            <h2 id="unit">How do I get the distance units I want?</h2>
            <p>If you need the metric displayed in the distance weights threshold box to reflect a particular unit (e.g., feet or miles), project your <a href="./glossary#shape">shape file</a> or other spatial file first and set the desired units in another program such as a <a href="./glossary#gis">GIS</a>. Then create the weights based on this file.</p>

            <h2 id="isle">What to do with islands?</h2>
            <p>Islands should not be used for LISA maps and can be problematic for GeoDa's regression models. One option to ensure no islands is to use distance weights (distance bands or k-nearest neighbors). You can also remove islands in GeoDa by exporting a new spatial file without islands or by assigning them to other areas that are similar by editing the weights matrix in a text editor (for details on the weights formats, see the <a href="/docs/geoda093.pdf" target="_blank" rel="noopener noreferrer">GeoDa 0.9.3 User's Guide</a>, pp. 80-81). You will need to assign a mainland area ID to the island ID, and vice versa, assign the island ID to the mainland ID.</p>

            <h2 id="projection">How do I assign projections and coordinate systems to my spatial file?</h2>
            <p>GeoDa does not contain any tools to set or change map projections or geographic coordinate systems. Geographic coordinate systems are relevant for adding a basemap to your map (you need WGS84). They can be relevant when using GeoDa's distance weights, which are based on the distances between points. You can set the distance units displayed in the weights distance dialog (e.g., feet, meters, or miles) by first projecting the spatial file in a <a href="./glossary#gis">GIS</a> outside of GeoDa.</p>

            <h2 id="lisactr">LISA maps only show cluster center</h2>
            <p>Note that the LISA cluster map only shows the <i>center</i> of the cluster in color (e.g., red for a high-high cluster). The actual extend of the cluster includes the center and the surrounding neighbors as defined by the <a href="./glossary#wghts">weights matrix</a>. If you are not sure which units are neighbors, use the connectivity map in the Weights Editor.</p>

            <h2 id="lisasig">How to assess the sensitivity of LISA results?</h2>
            <p>To assess the sensitivity of your results, compare several <a href="./glossary#perm">permutations</a> and apply several significance filters. This way you can explore which clusters and spatial outliers remain stable throughout.</p>

            <h2 id="lisabox">My box map does not reflect my LISA map - why?</h2>
            ...
            <h2 id="robust">How robust is the pseudo p-value?</h2>
            <p>Explore how robust your observed Moran's I value is compared to the <a href="./glossary#random">randomized</a> reference distribution. In the <a href="./glossary#gmoran">Moran scatter plot</a>, increase the number of permutations from 99 up and test if the relationship between the observed and randomized Moran's I changes. In the <a href="./glossary#lisa2">LISA</a> maps, increase the number of randomizations from 99 permutations up to see if the clusters change.</p>

            <h2 id="smooth">How to assess the sensitivity of different smoothers?</h2>
            <p>To assess how sensitive the identification of outliers is to a specific <a href="./glossary#smooth">smoothing</a> technique, you can visually compare the different smoothers in GeoDa. Create four maps (raw rate, EB-smoothed, spatial rate, and EBS-smoothed) and choose the <a href="./glossary#boxmap">box map</a> option under map themes when you choose your <a href="./glossary#event">base and event variables</a>.</p>
            ...
            <p>You can also open one or more <a href="./glossary#boxplot">box plot(s)</a> of the raw rate and/or the smoothed rates to explore the differences in <a href="./glossary#outliers">outliers</a> through linking and brushing.</p>

            <h2 id="regout">Run regression without opening project</h2>
            ...
            <h2 id="output">Where can I get help interpreting the regression output?</h2>
            <p>The <a href="/docs/geodaworkbook.pdf" target="_blank" rel="noopener noreferrer">GeoDa workbook</a> contains several regression chapters with more detail on interpretation of output. For further background reading, see the <a href="https://spatial.uchicago.edu/spatial-analysis-references" target="_blank" rel="noopener noreferrer">regression references</a> and, in particular, Anselin (1988).</p>
